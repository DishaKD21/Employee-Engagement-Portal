...
from app.services.rag import generate_local_answer, index_article, retrieve_best_chunk, score_confidence
...
def query_route(payload: QueryRequest, db: Session = Depends(get_db)):
    logger.info("RAG query request received", extra={"query_text": payload.query_text})
    best_chunk = retrieve_best_chunk(db, payload.query_text)

    if best_chunk is None:
        logger.info("RAG query returned no chunk match", extra={"query_text": payload.query_text})
    ...
    if confidence < settings.confidence_threshold:
        logger.info(
            "RAG query confidence below threshold",
            extra={"query_text": payload.query_text, "confidence": confidence, "article_id": best_chunk.article_id},
        )
        return QueryResponse(answer=None, confidence=confidence, matched_article_id=None, escalate=True)

    response = QueryResponse(
        answer=generate_local_answer(payload.query_text, best_chunk),
